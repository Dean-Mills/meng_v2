...
\begin{compactitem}
  \item After the adapter we don't care where the data came from, the rest of the pipeline is identical
...
\texttt{calculate\_distance\_to\_camera} was removed --- redundant now that we have per-joint $z$.
...
\subsubsection{What we tried and why it kept failing}
...
Rather than discarding the modularity objective in favour of pure supervision, we combine both:
\begin{equation}
...
All previous no-depth experiments were evaluated on synthetic data only. Before trying more loss engineering, test what we have on real data.
