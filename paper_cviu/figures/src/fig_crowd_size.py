...
Run from code_v3/:
    python ../paper_cviu/figures/src/fig_crowd_size.py
"""
...
def main():
    ...
    ax1.set_ylabel("end-to-end PGA", fontsize=8)
    ax1.set_ylim(0.6, 1.01)
    ax1.legend(frameon=False, fontsize=8, loc="lower left")
    ax1.set_title("(a) grouping accuracy on matched detections", fontsize=8)
    ...
    ax2.set_ylabel("AP@OKS", fontsize=8)
    ax2.set_ylim(0.0, 0.85)
    ax2.legend(frameon=False, fontsize=8, loc="lower left")
    ax2.set_title("(b) system-level AP on identical joints", fontsize=8)
    ax2.set_xticks(x)
    ax2.set_xticklabels([f"{l}\n$n$={n}" for l, n in zip(labels, n_ap)], fontsize=8)
    ax2.set_xlabel("annotated people in the image", fontsize=8)
