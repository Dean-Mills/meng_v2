%% Concept figure for the introduction: joint-trained grouping vs the modular PG-GAT pipeline.
%% Two stacked strips so the natural width stays within the 522 pt text width (fonts >= 8 pt unscaled).
...
\end{scope}
\node[dlbl, blue!60!black, anchor=north] at (jt.south) {one network, trained together on one detector's features};
\node[dlbl, anchor=west, align=left] at ($(posea.east)+(5mm,0)$)
  {grouping tied to its detector;\\ swapping the detector\\ means retraining the grouping};

%% ================= (b) modular grouping (bottom strip) =================
\node[hdr] (hb) at ($(jt.south west)+(0,-11mm)$) [anchor=north west, xshift=-2.5mm] {(b) modular grouping (this work)};
...
\node[inner sep=0, right=5mm of kp, anchor=west] (ins1) {\includegraphics[height=18mm]{../concept_inset_ungrouped.pdf}};
...
\node[ours, text width=19mm, anchor=west] (pg) at ($(ifx |- yolo.east)+(9mm,0)$) {PG-GAT\\ \textnormal{\small (frozen)}};
...
\node[inner sep=0, anchor=west] (ins2) at ($(pg.east)+(11mm,0)$) {\includegraphics[height=18mm]{../concept_inset_grouped.pdf}};
\draw[arr] (km.east) -- ++(6.5mm,0) |- (ins2.west);
\node[dlbl, anchor=north] at ($(ins2.south)+(0,-0.5mm)$)
  {grouped poses: one frozen module,\\ reused zero-shot behind each detector};
