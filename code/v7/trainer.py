...
class Trainer:
    ...
    def __init__(self, model, preprocessor, device="cuda"):
        ...
        self.optimizer = None  # Will be set in train()
        self.current_epoch = 0

    def save_checkpoint(self, epoch, is_final=False):
        """
        Save model checkpoint with weights, optimizer state, and training history.
        
        Args:
            epoch: Current epoch number
            is_final: If True, save as 'model_final.pt'
        """
        checkpoint = {
            "epoch": epoch,
        ...
        if is_final:
            path = self.checkpoint_dir / "model_final.pt"
        else:
    ...
    def train(self, dataloader, resume_from=None):
        ...
        print(f"Model saved to: {self.checkpoint_dir / 'model_final.pt'}")
    ...
    def plot_training_curves(self):
        ...
        ax2.plot(
            self.history["positive_distance"],
            label="Same Person (Pos Sim)",
            color="green",
            linewidth=2,
        )
        ax2.plot(
            self.history["negative_distance"],
            label="Different Person (Neg Sim)",
            color="red",
            linewidth=2,
        )
        ...
        final_pos = (
            self.history["positive_distance"][-1]
            if self.history["positive_distance"]
            else 0.0
        )
        final_neg = (
            self.history["negative_distance"][-1]
            if self.history["negative_distance"]
            else 0.0
        )
        final_sep = final_pos - final_neg

        ax4.text(
            0.1, 0.72, f"Final Loss: {final_loss:.4f}", fontsize=12, transform=ax4.transAxes
        )
        ax4.text(
            0.1, 0.52, f"Final Pos Sim: {final_pos:.4f}", fontsize=12, transform=ax4.transAxes
        )
        ax4.text(
            0.1, 0.32, f"Final Neg Sim: {final_neg:.4f}", fontsize=12, transform=ax4.transAxes
        )
        ax4.text(
            0.1,
            0.12,
            f"Separation (Pos-Neg): {final_sep:.4f}",
            fontsize=12,
