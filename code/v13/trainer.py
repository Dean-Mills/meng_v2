...
class Trainer:
    ...
    def __init__(
        ...
        if self.config.mode == "gat_only":
            ...
            loss_config = LossConfig(
                ...
                lambda_count=self.config.lambda_count,
                lambda_auxiliary=self.config.lambda_auxiliary,
    ...
    def train_epoch_full(self, dataloader) -> Dict[str, float]:
        ...
        for batch in dataloader:
            ...
            for graph in graphs:
                ...
                metrics["count_loss"] += loss_dict['count_loss'].item()
                aux = loss_dict['auxiliary_loss']
                metrics["auxiliary_loss"] += aux.item() if isinstance(aux, torch.Tensor) else aux
                metrics["count_pred"] += loss_dict['count_pred']
                metrics["pos_similarity"] += loss_dict['pos_similarity']
                metrics["neg_similarity"] += loss_dict['neg_similarity']
    ...
    def train(self, dataloader, resume_from: Optional[str] = None):
        ...
        # Setup optimizer
        if cfg.mode == "full" and hasattr(self.model, 'get_param_groups'):
            # Use separate LR for person queries (they need to diverge faster)
            param_groups = self.model.get_param_groups(
                base_lr=cfg.learning_rate,
                query_lr_multiplier=cfg.query_lr_multiplier
            )
        ...
        if cfg.mode == "full":
            print(f"Loss weights: exist={cfg.lambda_existence}, "
                  ...
                  f"count={cfg.lambda_count}, aux={cfg.lambda_auxiliary}")
            print(f"Query LR multiplier: {cfg.query_lr_multiplier}x")
        ...
        for epoch in range(start_epoch, cfg.epochs):
            ...
            self.history["count_loss"].append(metrics.get("count_loss", 0.0))
            self.history["auxiliary_loss"].append(metrics.get("auxiliary_loss", 0.0))
            self.history["count_pred"].append(metrics.get("count_pred", 0.0))
    ...
    def _print_epoch_summary(self, epoch: int, metrics: Dict[str, float], lr: float):
        ...
        if cfg.mode == "gat_only":
            ...
            print(
                ...
                f"Cnt:{metrics['count_loss']:.3f}) | "
                f"Count: {metrics['count_pred']:.1f} | "
    ...
    def _plot_full_curves(self):
        ...
        axes[0, 1].plot(epochs, self.history["count_loss"], 'orange', 
                       linewidth=2, label="Count")
        axes[0, 1].plot(epochs, self.history["auxiliary_loss"], 'brown', 
                       linewidth=2, label="Auxiliary", linestyle='--')
        ...
        # Matched
        axes[1, 0].plot(epochs, self.history["num_matched"], 'orange', linewidth=2)
        axes[1, 0].set_title("Avg Matched People")
        axes[1, 0].set_xlabel("Epoch")
        axes[1, 0].grid(True, alpha=0.3)
        
        # Count prediction
        axes[1, 1].plot(epochs, self.history["count_pred"], 'b-', linewidth=2, label="Predicted")
        axes[1, 1].set_title("Avg Count Prediction")
        axes[1, 1].set_xlabel("Epoch")
        axes[1, 1].legend()
        axes[1, 1].grid(True, alpha=0.3)
        
        # Count loss
        axes[1, 2].plot(epochs, self.history["count_loss"], 'r-', linewidth=2)
        axes[1, 2].set_title("Count Loss")
        axes[1, 2].set_xlabel("Epoch")
        axes[1, 2].grid(True, alpha=0.3)
        
        # LR
        axes[2, 0].plot(epochs, self.history["learning_rate"], 'orange', linewidth=2)
        ...
        # NMI
        axes[2, 1].plot(epochs, self.history["nmi"], 'teal', linewidth=2)
        axes[2, 1].set_title("NMI")
        ...
        neg = np.array(self.history["neg_similarity"])
        separation = pos - neg
        self._add_summary_panel(axes[2, 2], separation, full_mode=True)
    ...
    def _add_summary_panel(self, ax, separation, full_mode=False):
        ...
        if full_mode:
            lines.extend([
                ...
                f"Count Loss: {self.history['count_loss'][-1]:.4f}",
                f"Aux Loss: {self.history['auxiliary_loss'][-1]:.4f}",
                f"Avg Count Pred: {self.history['count_pred'][-1]:.2f}",
            ])
