...
class LossConfig:
    ...
    lambda_auxiliary: float = 1.0  # weight for intermediate layer losses
...
class PoseGroupingLoss(nn.Module):
    ...
    def _compute_layer_losses(
        ...
        """
        Compute existence, assignment, and count losses for a single layer's outputs.
        Used for both the final layer and intermediate auxiliary layers.
        """
        # Hungarian matching
        ...
        l_exist = self.existence_loss(
            outputs['existence_logits'],
            pred_indices, gt_indices, num_gt_people
        )
        
        l_assign = self.assignment_loss(
            outputs['assignment_scores'],
            outputs['joint_indices_per_type'],
            pred_indices, gt_indices, person_labels
        )
        
        l_count = self.count_loss(outputs['count_pred'], num_gt_people)
    ...
    def forward(
        ...
        """
        Compute all losses including auxiliary losses from intermediate layers.
        ...
        Args:
            outputs: Dict from PoseGroupingModel.forward()
                ...
                - embeddings: [N, D]
                - auxiliary_outputs: list of dicts from intermediate decoder layers
        ...
        # === Final layer losses ===
        final_losses = self._compute_layer_losses(
            outputs, person_labels, joint_types, num_gt_people
        )
        
        l_exist = final_losses['existence_loss']
        l_assign = final_losses['assignment_loss']
        l_count = final_losses['count_loss']
        
        # Contrastive loss (only on embeddings, not per-layer)
        ...
        aux_loss = torch.tensor(0.0, device=person_labels.device)
        auxiliary_outputs = outputs.get('auxiliary_outputs', [])
        
        if auxiliary_outputs and cfg.lambda_auxiliary > 0:
            for aux_out in auxiliary_outputs:
                aux_losses = self._compute_layer_losses(
                    aux_out, person_labels, joint_types, num_gt_people
                )
                aux_loss += (
                    cfg.lambda_existence * aux_losses['existence_loss'] +
                    cfg.lambda_assignment * aux_losses['assignment_loss'] +
                    cfg.lambda_count * aux_losses['count_loss']
                )
            
            # Average over intermediate layers
            aux_loss = aux_loss / len(auxiliary_outputs)
            total_loss += cfg.lambda_auxiliary * aux_loss
        ...
        return {
            ...
            'num_matched': final_losses['num_matched'],
