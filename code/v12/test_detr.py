...
def train_detr_standalone(
    ...
    loss_config = LossConfig(
        ...
        lambda_count=2.0,
        lambda_auxiliary=1.0,    # Intermediate layer supervision
        contrastive_margin=0.5,
        label_smoothing=0.0,
    )
    criterion = PoseGroupingLoss(loss_config)
    
    # ── Optimizer (separate LR for person queries) ─────────────────
    ...
    print(f"  1. Orthogonal query initialization")
    print(f"  2. Separate query LR: {lr * query_lr_multiplier:.1e} ({query_lr_multiplier}x)")
    print(f"  3. Intermediate layer auxiliary losses (lambda={loss_config.lambda_auxiliary})")
    print(f"  4. Count head (lambda={loss_config.lambda_count})")
    print("=" * 70)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for sample in batch:
            ...
            epoch_losses['auxiliary'] += loss_dict['auxiliary_loss'].item() if isinstance(loss_dict['auxiliary_loss'], torch.Tensor) else loss_dict['auxiliary_loss']
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            history['auxiliary_loss'].append(epoch_losses['auxiliary'])
            ...
            print(
                ...
                f"(E:{epoch_losses['existence']:.3f} "
                f"A:{epoch_losses['assignment']:.3f} "
                f"Cnt:{epoch_losses['count']:.3f} "
                f"Aux:{epoch_losses['auxiliary']:.3f}) | "
                f"PGA: {metrics['pga']:.4f} | "
                f"Count: {metrics['perfect_count_rate']:.0%} perfect, "
...
def plot_results(history: Dict, output_dir: Path):
    ...
    fig.suptitle("DETR Decoder Test (Orthogonal Init + Query LR + Aux Losses)", fontsize=14, fontweight='bold')
    ...
    ax.plot(epochs, history['auxiliary_loss'], 'purple', label='Auxiliary', linewidth=1.5, linestyle='--')
