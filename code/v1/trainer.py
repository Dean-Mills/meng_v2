...
class Trainer:
    ...
    def __init__(self, model, preprocessor, device='cuda'):
        ...
        self.device = device
        self.output_dir = settings.output_dir / "v1" 
    ...
    def contrastive_loss(self, embeddings, person_labels, margin=1.0):
        """
        ...
        for batch in dataloader:
            ...
            for graph in graphs:
                ...
                loss, pos_dist, neg_dist = self.contrastive_loss(
                    embeddings, graph.person_labels, margin=1.0
                )
    ...
    def plot_training_curves(self):
        ...
        # Distance curves
        ax2.plot(self.history['positive_distance'], label='Same Person', color='green', linewidth=2)
        ax2.plot(self.history['negative_distance'], label='Different Person', color='red', linewidth=2)
        ...
        # Distance difference (what we want to maximize)
        distance_diff = np.array(self.history['negative_distance']) - np.array(self.history['positive_distance'])
        ax3.plot(distance_diff, color='blue', linewidth=2)
        ax3.set_title('Distance Separation')
        ax3.set_xlabel('Epoch')
        ax3.set_ylabel('Neg Distance - Pos Distance')
        ax3.grid(True, alpha=0.3)
        
        # Summary stats
        final_loss = self.history['loss'][-1] if self.history['loss'] else 0
        final_pos = self.history['positive_distance'][-1] if self.history['positive_distance'] else 0
        final_neg = self.history['negative_distance'][-1] if self.history['negative_distance'] else 0
        
        ax4.text(0.1, 0.7, f"Final Loss: {final_loss:.4f}", fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.5, f"Final Pos Dist: {final_pos:.4f}", fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.3, f"Final Neg Dist: {final_neg:.4f}", fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.1, f"Separation: {final_neg - final_pos:.4f}", fontsize=12, transform=ax4.transAxes)
