...
class PersonCountHead(nn.Module):
    """
    Predicts the number of people by looking at ALL person query features together.
    
    Unlike the existence head (which decides per-query in isolation), this head
    pools all query features and predicts a single scalar count. This gives the
    model a global signal for how many people are in the scene.
    
    Input: [M, D] person features (all queries)
    ...
    def forward(self, person_features: torch.Tensor) -> torch.Tensor:
        """
        Args:
            person_features: [M, D] features from decoder (all queries)
        Returns:
            count_pred: scalar predicted count (clamped to [0, max_people])
        """
        # Mean pool across all queries: [M, D] -> [D]
        pooled = person_features.mean(dim=0)
        
        # Predict count
        count = self.mlp(pooled).squeeze(-1)
...
class DETRDecoder(nn.Module):
    ...
    def __init__(self, config: Optional[DETRConfig] = None):
        ...
        # Count head - predicts total number of people from all queries
    ...
    def forward(
        ...
        # Count prediction (global, looks at all queries together)
        count_pred = self.count_head(person_features)  # scalar
