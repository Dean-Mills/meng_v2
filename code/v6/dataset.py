...
class CocoKeypointsDataset(Dataset):
    def __init__(
        self, split: Literal["val", "train"] = "val"
    ):
        """
        Args:
            split: 'train' or 'val'
            num_people_required:
                -1 = include any number of people (skip if 0)
                >0 = require exactly this many people with all keypoints visible
        """
        self.split = split
        self.num_people_required = settings.num_people_required 

        if split == "val":
            self.img_dir = settings.coco_val_dir
            self.ann_file = os.path.join(
                settings.coco_annotations_dir, "person_keypoints_val2017.json"
            )
        elif split == "train":
            self.img_dir = settings.coco_train_dir
            self.ann_file = os.path.join(
                settings.coco_annotations_dir, "person_keypoints_train2017.json"
            )
        else:
        ...
        if self.num_people_required == -1:
            self.img_ids = [
                img_id
                for img_id in all_img_ids
                if self._has_at_least_one_person(img_id)
            ]
        else:
            self.img_ids = [
                img_id
                for img_id in all_img_ids
                # if self._has_exactly_n_visible_people(img_id, self.num_people_required)
                if self._has_at_least_n_people(img_id, self.num_people_required)
            ]

        print(f"Loaded {len(self.img_ids)} valid images for split '{split}'")

    def __len__(self):
        return len(self.img_ids)

    def _has_at_least_one_person(self, img_id):
        ...
        return len(person_anns) > 0

    def _has_exactly_n_visible_people(self, img_id, n):
        ann_ids = self.coco.getAnnIds(imgIds=img_id, iscrowd=False)
        anns = self.coco.loadAnns(ann_ids)
        person_anns = [
            ann
        ...
        if len(person_anns) < n:
            return False
        selected = person_anns[:n]
        for ann in selected:
    ...
    def _has_at_least_n_people(self, img_id, n):
        ann_ids = self.coco.getAnnIds(imgIds=img_id, iscrowd=False)
        anns = self.coco.loadAnns(ann_ids)
        person_anns = [
            ann for ann in anns
    ...
    def _transform_data(self, image, keypoints_list):
        ...
        new_width, new_height = int(original_width * scale), int(
    ...
    def visualize_item(self, idx, save_to_file=True):
        ...
        plt.imshow(img_np)
        plt.title(
            f"Image ID: {img_id} (Transformed to {settings.target_image_size}x{settings.target_image_size})"
        )
        plt.axis("off")
