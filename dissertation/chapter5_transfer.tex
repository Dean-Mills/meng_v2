...
\begin{figure}[H]
    ...
    \caption[Synthetic-validation against COCO-validation PGA for every configuration reporting both.]{Synthetic-validation PGA against COCO-validation PGA
    (ground-truth keypoints, $k$-means) for the 27 configurations of Chapter~\ref{chap_4} that report both.
    Points below the dashed diagonal lose accuracy in transfer;
    the vertical distance to the diagonal is the transfer gap.
    Arrows join the three synthetic-only models to their COCO fine-tuned versions.
    Values are the Chapter~\ref{chap_4} table cells -
    TriGAT is scored on $2{,}180$ scenes, every other configuration on $2{,}307$.}
