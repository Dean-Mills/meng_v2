"""Dissertation variant (6.1 in wide, 8.5 pt fonts, wrapped first-row titles; include at \linewidth).
...
The graph PG-GAT groups is built from the matched detections and the K-head predicts from it, exactly as
in the dissertation's detector-swap evaluation (eval_e2e_generic.py), so K-hat and PGA follow Table 4.11's
protocol rather than the deployable every-pooled-detection setting the paper's copy of this script used.

Run from code_v3/:
    python ../dissertation/figures/src/fig_detector_swap.py [--img_id 281759]
"""
...
plt.rcParams.update({
    ...
    "mathtext.fontset": "stix", "font.size": 8.5, "figure.dpi": 150,
})
...
def main():
    ...
    ap.add_argument("--img_id", type=int, default=470779)
    ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--suffix", default="", help="output stem suffix, e.g. _cand1 (candidate scenes)")
    a = ap.parse_args()
    ...
    with torch.no_grad():
        for name, fn in detectors:
            ...
            gt = m_person
            # the dissertation's swap protocol (eval_e2e_generic.py): the graph PG-GAT groups is built from the
            # detections matched to ground truth, and the K-head predicts from that graph
            g = build_graph_from_detections(pos[m_idx], typ[m_idx], pre, a.device).to(a.device)
            emb = gat(g)
            K_pred = max(1, min(int(k_head.predict(emb)), len(m_idx)))
            labels = predict_knn(emb, K_pred).cpu().numpy()
    ...
    panel_w = 1.68
    fig = plt.figure(figsize=(5.4, 3 * (panel_w * aspect + 0.36) + 0.15))
    gs = fig.add_gridspec(3, 3, left=0.06, right=0.995, top=0.95, bottom=0.01, wspace=0.03, hspace=0.24)
    for r, row in enumerate(rows):
        panels = [("(a) pooled keypoints,\nidentities discarded" if r == 0 else "pooled keypoints", True, row["native"]),
                  (f"(b) native grouping,\nPGA {row['pga_native']:.2f}" if r == 0 else f"native grouping, PGA {row['pga_native']:.2f}", False, row["native"]),
                  (f"(c) PG-GAT, $\\hat{{K}}={row['K_pred']}$,\nPGA {row['pga_pg']:.2f}" if r == 0 else f"PG-GAT, $\\hat{{K}}={row['K_pred']}$, PGA {row['pga_pg']:.2f}", False, row["labels"])]
        for c, (title, grey, labels) in enumerate(panels):
            ax = fig.add_subplot(gs[r, c])
            draw(ax, img_rgb, row["pos"], row["types"], labels, row["gt"], crop, title, grey=grey)
            if c == 0:
                ax.text(-0.02, 0.5, row["name"], transform=ax.transAxes, rotation=90, fontsize=8.5, ha="right", va="center")
    stem = FIGDIR / f"detector_swap{a.suffix}"
