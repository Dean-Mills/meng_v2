"""Dissertation variant (6.1 in wide, 8.5 pt fonts, three-line titles; include at \linewidth).
...
Run from code_v3/ (repo imports):
    python ../dissertation/figures/src/fig_embedding_space.py --proj pca
    python ../dissertation/figures/src/fig_embedding_space.py --proj tsne
Writes dissertation/figures/embedding_space_<proj>.pdf and .png (preview) and a JSON sidecar.
"""
...
def main():
    ...
    nrows = len(projs) + 1
    fig = plt.figure(figsize=(6.1, 1.85 * nrows + 0.45))
    gs = fig.add_gridspec(nrows, 4, width_ratios=[1.3, 1, 1, 1], height_ratios=[1.2] * (nrows - 1) + [1],
                          left=0.01, right=0.93, top=0.89, bottom=0.03, wspace=0.42, hspace=0.22)
    ...
    ax.set_title(f"COCO val2017 {chosen['img_id']}\n$K={K}$, {len(gt)} labelled keypoints", fontsize=8.5)
    ...
    for c, (lab, gat, pre, dim, mean, run) in enumerate(models, start=1):
        ...
        if c == 1:
            axb.set_ylabel("cosine similarity\n(ordered by person)", fontsize=8.5, labelpad=6)
        if c == 3:
    ...
    def cos_stats(emb):
        sim = emb @ emb.T
        same = gt[:, None] == gt[None, :]
        off = ~np.eye(len(gt), dtype=bool)
        return float(sim[same & off].mean()), float(sim[~same].mean())
    json.dump({"img_id": chosen["img_id"], "K": K, "n_nodes": int(len(gt)), "proj": a.proj, "projections": projs,
               "checkpoints": [{"label": m[0], "path": p[1], "coco_val_pga": m[4], "run": m[5], "scene_pga": chosen["pga"][i],
                                ...
                                "mean_cos_within_person": cos_stats(chosen["emb"][i])[0],
                                "mean_cos_cross_person": cos_stats(chosen["emb"][i])[1]}
                               for i, (m, p) in enumerate(zip(models, CHECKPOINTS))]},
              open(stem.with_suffix(".json"), "w"), indent=2)
    for i, m in enumerate(models):
        w, x = cos_stats(chosen["emb"][i]); print(f"{m[0]:<24s} mean cosine within-person {w:.3f}  cross-person {x:.3f}")
