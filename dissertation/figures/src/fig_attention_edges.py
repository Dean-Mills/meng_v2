"""Dissertation variant (6.1 x 6.0 in, panels (a)(b) above (c), 9 pt fonts; include at \linewidth).
...
Run from code_v3/:
    python ../dissertation/figures/src/fig_attention_edges.py [--img_id 554328] [--max_scenes N]
Writes dissertation/figures/attention_edges.pdf/.png and a JSON sidecar with every number plotted.
"""
...
def draw(a, out, scene_c, loaded, stem):
    ...
    gs = fig.add_gridspec(2, 2, width_ratios=[1.35, 1.0], height_ratios=[1.0, 1.25], left=0.09, right=0.98, top=0.95, bottom=0.06, wspace=0.32, hspace=0.42)
    ...
    ax.set_ylabel("share of attention mass", fontsize=9)
    ax.set_title("(a) attention by edge category (layer average)", fontsize=9)
    ax.legend(frameon=False, fontsize=9, loc="upper left", handlelength=1.2, borderaxespad=0.2)
    ...
    for lab, col, mk, short, kk in ((base, "0.45", "s", "Standard GAT", 8), (pgl, "#c23b3b", "o", "PG-GAT", 16)):
        ...
        ax.text(xs[-1] + 0.1, ys[-1], short, fontsize=9, color=col, va="center")
        ax.text(xs[-1] + 0.1, us[-1], f"uniform, $k$={kk}", fontsize=9, color=col, va="center")
    ax.set_xlabel("layer", fontsize=9); ax.set_xticks([1, 2, 3]); ax.set_xlim(0.8, 4.3)
    ax.set_ylabel("share of non-self attention\non same-person edges", fontsize=9)
    ax.set_title("(b) attention within the person", fontsize=9)
    ...
    if scene_c is not None:
        ...
        ax.legend(handles=[Line2D([], [], color=CAT_COLORS[c], lw=2, label=CATS[c].replace("\n", " ")) for c in range(4)],
                  frameon=False, fontsize=9, loc="upper center", bbox_to_anchor=(0.5, -0.01), ncol=4,
                  handlelength=1.2, columnspacing=1.2, handletextpad=0.5)
        ax.set_title("(c) top-2 attended edges per keypoint, last layer", fontsize=9)
    ax.axis("off")
