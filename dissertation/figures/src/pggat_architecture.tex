...
\draw[arr] (cat.east) -- ++(2mm,0) |- (edgecat);
\draw[arr] (geo.east) -- ++(2mm,0) |- (edgecat);
\draw[arr] (edgecat) -- (enc);

%% edge features enter the attention of every GAT layer
\draw[arr] (enc.east) -| (gatbox.south)
  node[pos=0.85, right=2pt, align=left, font=\small]{additive attention bias\\ (every layer)};
