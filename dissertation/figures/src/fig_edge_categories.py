"""Dissertation variant (figsize 4.3 x 3.75 in, 9 pt fonts; include at 0.7\linewidth of the 441 pt text width).
...
Run from code_v3/:
    python ../dissertation/figures/src/fig_edge_categories.py
"""
...
def main():
    ...
    fig, ax = plt.subplots(figsize=(4.3, 3.75))
    ...
    ax.annotate("anchor:\nL shoulder", A[ANCHOR], textcoords="offset points", xytext=(-9, -2), fontsize=9, ha="right", va="center")
    ...
    for j in range(5, 17):
        ...
        ax.annotate(f"{int(hops[ANCHOR, j])}", A[j], textcoords="offset points", xytext=(dx, 0), fontsize=9,
    ...
    ax.text(A[0, 0], A[0, 1] - 0.16, "person A", ha="center", fontsize=9)
    ax.text(B[0, 0], B[0, 1] - 0.16, "person B", ha="center", fontsize=9)
    ...
    ax.legend(handles=handles, frameon=False, fontsize=9, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=3,
