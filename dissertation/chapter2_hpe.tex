...
This is a large domain, so the work is restricted to 2D and multi-person.
...
The following three useful surveys provide a reader with considerable background and scope:
...
so Hungarian matching at training time assigns each ground-truth person to one of the queries.
...
introducing a differentiable soft-argmax that bridged heatmap and regression paradigms.
DSNT~\cite{nibali2018dsnt} later formalised the same idea as a normalised spatial-coordinate layer.

RLE~\cite{li2021rle} offered a more direct probabilistic treatment,
done by modelling residuals between predicted and ground-truth coordinates with a flow-based density
...
PCT~\cite{geng2023pct} takes another approach
...
In comparison, the literature on \emph{modular} grouping is much smaller.
