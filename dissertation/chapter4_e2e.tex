...
\begin{figure}[H]
    ...
    \caption[Predicted against annotated person count for the K-head on HigherHRNet detections.]{Confusion matrix of the predicted person count $\hat{K}$ against the annotated count $K$
    for the $2{,}165$ scored HigherHRNet scenes.
    Counts of eight or more are pooled into the final row and column,
    and entries below the diagonal correspond to under-counting.
    The matrix is generated from a re-run of the evaluation pipeline
    that reproduces the reported aggregate PGA to within $0.0003$.
    Two fewer scenes match the annotated count than in the logged run,
    so the exact-count rate reads $76.0\%$ here against the reported $76.1\%$,
    and the within-one rate $92.1\%$ against $91.9\%$.}
...
The first scene has five people and is grouped correctly by all three.
The second is a grouping error at the correct count -
PG-GAT misassigns joints across two people while the K-head returns the annotated count.
In the third, one of the three annotated people has no matched detections,
so the annotated $K$ splits a represented person while the K-head's $\hat{K}=2$ does not.
In the fourth, the K-head under-counts and merges two people.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\linewidth]{figures/e2e_examples.pdf}
    \caption[Four scored COCO scenes under HigherHRNet AE grouping, PG-GAT with the annotated $K$, and PG-GAT with the predicted $\hat{K}$.]{Four scored COCO \texttt{val2017} scenes under the end-to-end evaluation protocol:
    (a) HigherHRNet's native AE grouping,
    (b) PG-GAT with $k$-means using the annotated person count $K$,
    and (c) PG-GAT with $k$-means using the K-head prediction $\hat{K}$.
    Only detections matched to ground-truth keypoints are shown.
    Detections are coloured according to the ground-truth person
    to which their predicted cluster is assigned by Hungarian matching;
    a keypoint that does not share its person's colour therefore indicates a grouping error.
    The PGA reported in each panel title is the corresponding per-scene value.}
...
Figure~\ref{fig:detector_swap} shows the detector swap on a single scene containing
five heavily overlapping skiers. Despite the differences in the detected keypoint sets,
the same frozen PG-GAT and K-head predict the annotated
count of five for all three detectors. The remaining grouping errors are small and occur
at different keypoints for each detector.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.875\linewidth]{figures/detector_swap.pdf}
    \caption[One scene under the detector swap for all three evaluated detectors.]{One scene (COCO \texttt{val2017} image 470779, five annotated people) under the detector-swap evaluation.
    Rows correspond to the three detectors, while columns show
    (a) the pooled keypoints after discarding the detector's person identities,
    (b) the detector's native grouping,
    and (c) the grouping produced by the frozen PG-GAT using the K-head prediction $\hat{K}$.
    Drawing conventions follow Figure~\ref{fig:e2e_examples}:
    only matched detections are shown, coloured by the ground-truth person to which their cluster is assigned.
    The PG-GAT graph is constructed from these matched detections ($59$, $66$, and $70$ keypoints),
    consistent with the evaluation in Table~\ref{tab:e2e_detector_swap}.}
    \label{fig:detector_swap}
\end{figure}