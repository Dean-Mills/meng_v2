...
def predict_dmon(
    ...
    """Run DMoN head, return [N] label tensor (argmax assignment)."""
    logits, s, *_ = head(embeddings, edge_index, k, joint_types=joint_types)
    return logits.argmax(dim=1)
...
class Evaluator:
    def __init__(self, checkpoint_path: Path, device: str = "cuda"):
        ...
        if ckpt.get("head_state") is not None:
            ...
            elif self.cfg.dmon is not None:
