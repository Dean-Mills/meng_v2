...
class DMoNLoss(nn.Module):
    """
    Combined loss for DMoN grouping head.

    Combines DMoN's structural losses (spectral, orthogonality, collapse) with
    supervised Hungarian-matched cross entropy and type exclusivity.

    The head computes structural and type losses internally. This class adds
    supervised CE and combines everything with configurable weights.

    Args:
        config:      LossConfig — uses dmon_weight for overall scaling.
        dmon_config: DMoNConfig — sub-weights for individual loss components.
    """

    def __init__(self, config: LossConfig, dmon_config: DMoNConfig):
        super().__init__()
        self.weight           = config.dmon_weight
        self.lambda_spectral  = dmon_config.lambda_spectral
        self.lambda_ortho     = dmon_config.lambda_ortho
        self.lambda_cluster   = dmon_config.lambda_cluster
        self.lambda_type      = dmon_config.lambda_type
        self.lambda_supervised = dmon_config.lambda_supervised
    ...
    def forward(
        ...
        """
        Args:
            logits:        [N, K]  assignment logits from DMoNHead
            person_labels: [N]     ground truth person ID per joint
            spectral_loss: scalar  from head
            ortho_loss:    scalar  from head
            cluster_loss:  scalar  from head
            type_loss:     scalar  from head
        ...
        # ── Hungarian-matched cross entropy (same as SlotAttentionLoss) ────
        unique_labels = torch.unique(person_labels)
        num_people    = len(unique_labels)
